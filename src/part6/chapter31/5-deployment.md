...
```bash
deno compile --allow-read --output mdify target/lykn/build/mdify/mod.js
```
...
deployctl deploy --project=shortn target/lykn/build/shortn/mod.js

# Self-hosted — just run it
deno run --allow-net --allow-read target/lykn/build/shortn/mod.js
```
...
```bash
cp index.html target/lykn/build/browser-app/mod.js /var/www/html/app.js
```
...
All three deployment targets share the same upstream pipeline: check, compile, lint, test, verify documentation. The only difference is the final step — what you do with the JavaScript under `target/lykn/build/` or the package staging under `target/lykn/dist/`. A CLI tool compiles to a binary. A server runs on Deno. A browser app ships as static files. The pipeline doesn't care which one you choose.
